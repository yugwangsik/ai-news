...
# 시간대 표기가 없는 날짜를 어느 지역 시각으로 볼지. AI타임스처럼 '2026-07-31
# 15:57:30' 형태로만 주는 피드가 있는데, 이를 UTC 로 오해하면 기사가 9시간
# 미래로 밀려 날짜 그룹이 하루 어긋납니다.
REGION_TZ_HOURS = {"KR": 9, "JP": 9, "US": 0}


def source_tzinfo(source):
    raw = source.get("tz")  # "+09:00" 형태로 소스별 지정 가능
    if raw:
        sign = -1 if raw[0] == "-" else 1
        hours, _, minutes = raw.lstrip("+-").partition(":")
        return timezone(sign * timedelta(hours=int(hours), minutes=int(minutes or 0)))
    return timezone(timedelta(hours=REGION_TZ_HOURS.get(source["region"], 0)))


def parse_date(raw, default_tz=timezone.utc):
    if not raw:
        return None
    raw = raw.strip()
    ...
    try:
        ...
        pass
    # ISO 8601 (Atom 의 published/updated, dc:date) 및 'YYYY-MM-DD HH:MM:SS'
    try:
...
def entry_date(entry, default_tz=timezone.utc):
    for name in ("pubdate", "published", "date", "updated", "modified"):
        dt = parse_date(text_of(entry, name), default_tz)
        if dt:
            return dt
    return None
...
def harvest(source, cutoff):
    ...
    default_tz = source_tzinfo(source)
    # 시계 오차나 예약 발행으로 살짝 미래인 건 허용하되, 크게 벗어나면 버립니다.
    horizon = datetime.now(timezone.utc) + timedelta(hours=6)
    items = []
    for entry in collect_entries(root):
        ...
        if not title or not link:
            continue
        published = entry_date(entry, default_tz)
        if published is None or published < cutoff or published > horizon:
            continue
        item = {
